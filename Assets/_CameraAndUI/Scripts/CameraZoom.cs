...
namespace Game.CameraUI{
	...
	{
		[SerializeField] bool _allowZoomEffect = false;
		...
		List<MyPlayer> _characters = new List<MyPlayer>();
		Camera _camera;
		CameraBoundariesLogic _logic;
		void Start()
		{
			_logic = new CameraBoundariesLogic();
			_characters = FindObjectsOfType<MyPlayer>().ToList();
		...
		[Task]
		bool ZoomEffectAllowed()
		{
			return _allowZoomEffect;
		}
